...
function FAQBlock({ faqData, withScroll = ScrollVariant.TO_TOP_BLOCK }: IFAQBlock) {
  const getFaqSEOData = () => faqData.map(({ title, content }) => ({
    question: renderToString(title as unknown as ReactElement)
      .replace(/<[^>]+>/g, '')
      .replace(/&#x27;/g, "'")
      .replace(/\s+/g, ' '),
  ...
  return (
    <>
      ...
      <div className="bg-black text-white p-[80px_0px] screen-lg:p-[80px_30px] screen-md:p-[80px_15px]">
        <MainTitle
        ...
        <AccordionBlack
          ...
          classes={{
            ...
            accordionItemTitle: `relative !p-[33px_80px] screen-xxxl:!p-[33px_70px] screen-lg:!p-[20px_0]
